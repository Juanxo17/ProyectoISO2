# app/adapters/chromadb_adapter.py
import chromadb
...
class ChromaDBAdapter(ports.DocumentRepositoryPort):
    def __init__(self, number_of_vectorial_results: int, api_key: str) -> None:
        ...
        self.collectionDocuments = self.client.create_collection(
            "documents"
        )  # Sin 'dimension'
    ...
    def save_document(
        ...
        if (
            len(flat_embeddings) != 1536
        ):  # Cambia esto para aceptar la dimensión correcta (1536)
            raise ValueError(
                f"Expected embeddings of dimension 1536, but got {len(flat_embeddings)}"
            )
    ...
    def get_document(self, doc_id: str) -> models.Document:
        ...
        return models.Document(
            ...
            user_id=vectors["metadatas"][0]["user_id"],
        )
